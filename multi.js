...
const createSession = function(id, description) {
  ...
  const message  = msg.body.toLowerCase(); 	
    ...
    client.sendMessage(msg.from, "Downloading..")
    TiktokDL(url, {
        ...
        try {
            if (result.result.type == 'video') {
                const ttu = JSON.parse(JSON.stringify(result.result.video2))
                const tt = await MessageMedia.fromUrl(ttu, {
                    unsafeMime: true
                })
                client.sendMessage(msg.from, tt)
            } else if (result.result.type == 'image') {
                let hasil = JSON.parse(JSON.stringify(result.result.images))
                
                for (let i = 0; i < hasil.length; i++) {
                    // console.log(result.result.author.nickname)
                    client.sendMessage(msg.from, await MessageMedia.fromUrl(hasil[i]), {caption: result.result.author.nickname})
                }

            }

        } catch (err) {
            const contactOne = await client.getContactById("6285654643240@c.us", {
                'parseVCards': true
            })
            client.sendMessage(msg.from, "Gagal download! ERORRNYA " + err.message)
        }
    })
} 
